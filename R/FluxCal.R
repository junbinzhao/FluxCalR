...
#' @param t A number indicates window width for linear regression slope calculation, unit: minutes.
#' @param spt A number indicates number of graphs to split the CO2 time series into for MANUALLY selecting the time cues. Cutting CO2 time series into several sections will aid the visualization and selection of the time cues when the data is too long. Default: 1.
#' @param Time_keys A file name (string) with times (HH:MM:SS) indicating start or end of each flux measurement.The header for the time must be either "Start" or "End". (see an example file "Time & Ta.csv" at https://github.com/junbinzhao/FluxCalR/tree/master/inst/extdata).
